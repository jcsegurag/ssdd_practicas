...
public class VideoFaces implements Runnable
{
	...
	public void run() {
		...
		//Video<InputStream> video = new XuggleVideo(new InputStream(daoVideo.getStreamForVideo(id)));
		
		//Video<InputStream> video = new XuggleVideo(videoStream);
		VideoDisplay<MBFImage> vd = VideoDisplay.createOffscreenVideoDisplay(videoImagen);
        // El Thread de procesamiento de vídeo se termina al terminar el vídeo.
        vd.setEndAction(EndAction.CLOSE_AT_END);
        logger.info("-----------------------------------------------------------------ESTAMOS ANTES DEL VIDEOLISTENER-----------------------------------------------------------------");
            ...
            public void beforeUpdate(MBFImage frame)
            {
                ...
                for (DetectedFace face : faces)
                {
                    ...
                    try
                    {
                        // También permite enviar la imagen a un OutputStream
                        ImageUtilities.write(frame.extractROI(face.getBounds()),
                                new File(String.format("/tmp/img%05d.jpg",  )));
                    } catch (IOException e)
                    {
                        // TODO Auto-generated catch block
                        e.printStackTrace();
